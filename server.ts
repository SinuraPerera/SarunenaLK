...
if (hfKey && hfKey !== "YOUR_HUGGINGFACE_API_KEY") {
  try {
    ...
    // Silent initialization failure - will use fallback
  }
}
...
Structure your response gracefully. Always answer the question directly. Note that you MUST answer ANY question the user asks, including general knowledge, calculations, translations, or technical advice — never refuse or lock yourself strictly to agriculture, but always maintain your extremely friendly and supportive Sri Lankan advisor persona.`;
  ...
  const requestedProvider = req.body.provider === 'huggingface' ? 'huggingface' : 'gemini';
  const activeAi = requestedProvider === 'huggingface' ? null : createGeminiClient(requestedApiKey);
  const activeHf = requestedProvider === 'gemini' ? createHuggingFaceClient(requestedApiKey) : createHuggingFaceClient(requestedApiKey);

  if (activeAi) {
    try {
      const prompt = `${systemPrompt}${context ? `\n\nFarmer context: ${JSON.stringify(context)}` : ""}\n\nConversation:\n${sanitizedMessages.map((msg: any) => `${msg.role === 'assistant' ? 'Assistant' : 'User'}: ${msg.text}`).join('\n')}`;
      ...
      // Fall through to the next provider or local fallback below.
    }
  }

  if (activeHf) {
    try {
      const conversationHistory = sanitizedMessages.map((msg: any) => ({
        role: msg.role === "assistant" ? "assistant" : "user",
        content: msg.text,
      }));
      ...
      for (const model of modelsToTry) {
        try {
          const response = await activeHf.chatCompletion({
            model: model,
...
*SarunenaLK Tip*: The live advisor is currently running in local guidance mode. Add a valid **GEMINI_API_KEY** or **HUGGINGFACE_API_KEY** in your environment to unlock real-time responses.
