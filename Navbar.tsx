    ...
    <nav className={`sticky top-0 z-50 ${themeColors.primaryBg} text-white shadow-md border-b-4 border-[#F9A825]`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          >
            <div className="bg-[#FFFDF7] p-1.5 sm:p-2.5 rounded-2xl shadow-inner transition-transform group-hover:scale-105 border-2 border-[#F9A825] overflow-hidden flex-shrink-0">
              <img src={logoUrl} alt="SarunenaLK logo" className="h-6 w-6 sm:h-7 sm:w-7 object-contain" />
            </div>
            <div className="ml-2 hidden sm:block min-w-0">
              <div className="flex items-center space-x-1.5">
                <span className="font-sans font-black text-base sm:text-xl tracking-tight leading-none text-[#FFFDF7]">
                  {t.title}
                </span>
          ...
          <div className="flex lg:hidden items-center space-x-1.5 sm:space-x-2">
            <div className="flex items-center bg-white/10 rounded-lg px-1.5 py-1 sm:px-2">
              <Globe className="h-3.5 w-3.5 text-white/80 mr-1" />
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value as Language)}
                className="bg-transparent text-[10px] sm:text-xs font-bold text-white focus:outline-none"
              >
